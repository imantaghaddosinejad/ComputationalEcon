...
% set transition path
%=========================
Tlen = 2001; % total length of transition path (+1 to anchor t=1 to steady-state)
...
%=========================
% extract parameters to reduce parfor overhead
%=========================
alpha = p.alpha;
delta = p.delta;
beta = p.beta;
...
while err_ge > tol_ge 
    ...
    %=========================
    % 2. backward solution (parallelized)
    %=========================
    % traditional backward induction (t=T:-1:1) not needed here. each
    % period independently uses previous GE iteration's full path, so 
    % direction is irrelevant and periods can be solved in parallel.
    % broadcast variables (K_t, L_t, mpolaprime_t, mlambda_t) from previous
    % iteration are read-only and safely shared across workers.
    parfor t = 1:T
        ...
        mexp = 0;
        for iAprime = 1:NA_local 
            Aprime = vgridA(iAprime); % future shock value 
            
            for izprime = 1:Nz_local
                ...
                if ifutureshock ~= iAprime || t == T
                    ...
                    rprime_low = alpha*Aprime*(K2Lprime_low)^(alpha-1)-delta;
                    wprime_low = (1-alpha)*Aprime*(K2Lprime_low)^alpha;
                    
                    K2Lprime_high = Kprime/L_t(canloc(Khigh));
                    rprime_high = alpha*Aprime*(K2Lprime_high)^(alpha-1)-delta;
                    wprime_high = (1-alpha)*Aprime*(K2Lprime_high)^alpha;
                    ...
                    nprime_low = -mprime_low/2 + sqrt((eta*mprime_low).^2 + 4*eta)/(2*eta); % future labour supply
                    ...
                    nprime_high = -mprime_high/2 + sqrt((eta*mprime_high).^2 + 4*eta)/(2*eta); % future labour supply
                    ...
                    mexp = mexp ...
                        + weightlow*((1+rprime_low)*muprime_low.*repmat(mtransz(:,izprime)',Na_local,1)*mtransA(iA,iAprime)) ...
                        + (1-weightlow)*((1+rprime_high)*muprime_high.*repmat(mtransz(:,izprime)',Na_local,1)*mtransA(iA,iAprime));
                ...
                % compute on the path expectation for realised future aggregate state  
                    ...
                    rprime = alpha*Aprime*(K2Lprime)^(alpha-1)-delta;
                    wprime = (1-alpha)*Aprime*(K2Lprime)^alpha;
                    ...
                    nprime = -mprime/2 + sqrt((eta*mprime).^2 + 4*eta)./(2*eta); % future labour supply 
                    ...
                    mexp = mexp ...
                        + (1+rprime)*muprime.*repmat(mtransz(:,izprime)',Na_local,1)*mtransA(iA,ifutureshock);
                end
            end
        end
        ...
        n = (w*mgridz)./(eta*c); % labour supply (using pre-computed matrix)
        ...
        mlambda_new_temp(mpolaprime_new_temp>agridmin) = 0;
        c = (c - (agridmin-mpolaprime_new_temp)).*(mpolaprime_new_temp<=agridmin) ...
            + c.*(mpolaprime_new_temp>agridmin);
        mpolaprime_new_temp(mpolaprime_new_temp<=agridmin) = agridmin;
...
fprintf('\n');
fprintf('======================== \n');
fprintf('Complete business cycle statistics on HP-filtered time series \n');
fprintf('======================== \n');
...
legend("True LoM","Linear LoM","location","best","FontSize",8);
ax = gca;
ax.FontSize = 15; 
set(gcf, 'Units', 'inches', 'Position', [1 1 5 4]);
